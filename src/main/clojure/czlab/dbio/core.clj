...
(defn dbGetO2M
  ...
  (when-some [r (dbioGetO2X ctx lhsObj :O2M)]
    (-> ^SQLr
        (:with ctx)
...
(defn dbGetO2O
  ...
  (when-some [r (dbioGetO2X ctx lhsObj :O2O)]
    (-> ^SQLr
        (:with ctx)
        (.findOne (or (:cast ctx)
                      (:other r))
